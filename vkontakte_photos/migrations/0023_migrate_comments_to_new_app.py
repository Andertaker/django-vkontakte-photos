...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        photo_ct = ContentType.objects.get(app_label='vkontakte_photos', model='photo').id

        comments = orm.Comment.objects.all()

        for c in comments:
            photo = orm.Photo.objects.get(id=c.photo_id)

            new_comment = Comment()
            new_comment.fetched = c.fetched
            new_comment.archived = c.archived
            new_comment.remote_id = c.remote_id
            new_comment.date = c.date
            new_comment.text = c.text

            new_comment.owner_id = photo.owner_id
            new_comment.owner_content_type_id = photo.owner_content_type_id

            new_comment.object_id = photo.remote_id
            new_comment.object_content_type_id = photo_ct

            new_comment.author_id = c.author_id
            new_comment.author_content_type_id = c.author_content_type_id
