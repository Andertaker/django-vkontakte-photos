...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        """
            Covert remote_id field from Char into BigInt type
            -16297716_284356567 -> 284356567

        """
        # convert album
        albums = list(orm.Album.objects.only('remote_id').all())

        db.delete_column('vkontakte_photos_album', 'remote_id')
        db.add_column('vkontakte_photos_album', 'remote_id', models.BigIntegerField(null=True))
        db.create_unique('vkontakte_photos_album', 'remote_id')

        for a in albums:
            a.remote_id = a.remote_id.split('_').pop()
            a.save()

        # convert photo
        photos = list(orm.Photo.objects.only('remote_id').all())

        db.delete_column('vkontakte_photos_photo', 'remote_id')
        db.add_column('vkontakte_photos_photo', 'remote_id', models.BigIntegerField(null=True))
        db.create_unique('vkontakte_photos_photo', 'remote_id')

        for p in photos:
            p.remote_id = p.remote_id.split('_').pop()
            p.save()
